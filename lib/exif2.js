...
async function stikermeta (media, metadata) {
    ...
    if (metadata.packname || metadata.author) {
        ...
        const json = { "sticker-pack-id": `https://github.com/rtwone/chitandabot`, "sticker-pack-name": metadata.packname, "sticker-pack-publisher": metadata.author, 'emojis': metadata.categories, 'is-avatar-sticker': 1 }
...
async function smetaVid (media, metadata) {
    let wMedia = await videoToWebp(media)
    const tmpFileIn = `./sticker/${Crypto.randomBytes(6).readUIntLE(0, 6).toString(36)}.webp`
    const tmpFileOut = `./sticker/${Crypto.randomBytes(6).readUIntLE(0, 6).toString(36)}.webp`
    fs.writeFileSync(tmpFileIn, wMedia)
...
module.exports = { imageToWebp, videoToWebp, writeExifImg, stikermeta, smetaVid, writeExifVid, writeExif }