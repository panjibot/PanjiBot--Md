/**
  ...
  * Follow me on Instagram @panjisubakti2
*/
...
const jadibot1 = async (conn, msg, q, from) => {
...
async function startVanz() {    
    ...
    if (usePairingCode && !conn.authState.creds.registered) {
        const code = await conn.requestPairingCode(q.trim())
        sendMessage(from, { text: `⚠︎ Kode Pairing Bot Whatsapp kamu\nNomer: ${q}\nPairing Code: ${code}\n\nJadi Bot Pairing Code By Vanz` })
    }
        store.bind(conn.ev)
	...
	conn.ev.on('connection.update', async (update) => {
        ...
        try {
            ...
            if (update.connection == "open" || update.receivedPendingNotifications == "true") {
                console.log(`[Connecting to] Bot WhatsApp`)
